...
**From Draft to Patch: Context-Aware Agents for Corrective Program Repair**

Refine is a context-aware framework that improves draft patches generated by
automated program repair (APR) systems. It disambiguates issue and code context,
diversifies candidate patches through test-time scaling, and uses an LLM-powered
review stage to aggregate partial fixes into more complete and robust patches.

The public repository is available at
[github.com/AnvithPabba/Refine-Program-Repair](https://github.com/AnvithPabba/Refine-Program-Repair).
The accompanying paper is included as [`preprint.pdf`](preprint.pdf), and the
...
Refine is designed as a general patch-refinement module for both open-agent and
workflow-based APR systems. In the paper's evaluation:

- Refine improves AutoCodeRover by 14.67 percentage points on SWE-bench Lite,
  reaching a 51.67% resolution rate.
- Refine improves the resolution rate by 12.2 percentage points on SWE-bench
  Verified.
- Across multiple APR systems, Refine provides an average improvement of 14%.

See the [preprint](preprint.pdf) for the experimental design and the
[`results/`](results/) directory for the published artifacts.
